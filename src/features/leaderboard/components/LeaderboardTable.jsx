...
/**
 * Realtime Ranked Telemetry Component
 * Subscribes dynamically to RLS-isolated organization profiles.
 */
export const LeaderboardTable = ({ department = null }) => {
  const { data: players, isLoading } = useRealtimeData('profiles', {
  ...
  if (isLoading) return <Spinner className="py-12" size="lg" />;

  if (filtered.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-text-muted">No combat records identified in this segment.</p>
      </div>
    );
  }
