...
/**
 * Historical Combat Feed Component
 * Visualizes realtime telemetry streams adhering to ABAC visibility policies.
 */
export const RecentMatches = () => {
  const user = useAuthStore((s) => s.user);

  const { data: matches, isLoading } = useRealtimeData('match_history', {
    ...
    <div className="glass rounded-xl p-5">
      ...
      {matches.length === 0 ? (
        <div className="text-center py-8">
          <p className="text-text-muted text-sm">No battles recorded yet. Forge your rank!</p>
        </div>
      ) : (
